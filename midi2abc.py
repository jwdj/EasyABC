...
def time_to_note_length(time):
    unit_time = 0.25 # corresponds to the length of a 16th note (one forth of a quarter which corresponds to 1.0)
...
def midi_to_abc(filename=None, notes=None, key=None, metre=Fraction(3, 4), default_len=Fraction(1, 16), bars_per_line=4, title='', source='', no_triplets=False, no_broken_rythms=False, slur_8th_pairs=False, slur_16th_pairs=False, slur_triplets=True, no_beam_breaks=False, index='1', anacrusis_notes=0):
    ...
    while notes:
        ...
        # else handle notes one by one or as a chord            
            ...
            is_four_16th_notes = len([n for n in notes[0:4] if n.length == Fraction(1, 16)]) == 4
...
def main(argv):
    ...
    result = midi_to_abc(filename, None, key, metre, default_len, bars_per_line, title, source, no_triplets, no_broken_rythms, slur_8th_pairs, slur_16th_pairs, no_beam_breaks=no_beam_breaks)
