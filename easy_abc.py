...
#FAU 20240105: pstopdf is not provided anymore by OS starting from sonoma
#FAU 20240105: integrating ghoscript shared library for Mac
gsapi_available = False
if wx.Platform == "__WXMAC__":
    try:
        import gsapi
        gsapi_available = True
    except ImportError:
        sys.stderr.write('Failure to import gsapi. Check shared library path')
...
# Added on 20240101 by F. Aupepin
# Distill temporary ps file generated by abcm2ps to pdf
# Change introduce as pstopdf not included in macos sonoma onwards
# using ghostscript shared library embedded according to demo interface gsapi
...
def distill_PSToPDF(in_filename, out_filename):
    global execmessages
...
def AbcToPDF(settings, abc_code, header, cache_dir, extra_params='', abcm2ps_path=None, gs_path=None, abcm2ps_format_path=None, generate_toc=False):
    ...
    # FAU20240101 : change to ghoscript shared library for Mac
    execmessages += '\n\nAbcToPDF\n'
    if wx.Platform == "__WXMAC__" and gsapi_available:
        distill_PSToPDF(ps_file, pdf_file)
    else:
        execmessages += " ".join(cmd2)
...
#FAU20240107 : use get default path to look for nwc2xml
    if not nwc2xml_path or not os.path.exists(nwc2xml_path):
        nwc2xml_path = get_default_path_for_executable('nwc2xml')
#        if wx.Platform == "__WXMSW__":
...
#            nwc2xml_path = 'nwc2xml'
    ...
    if not os.path.exists(nwc2xml_path) or returncode != 0:
...
class AbcFileSettingsFrame(wx.Panel):
    ...
    def __init__(self, parent, settings, statusbar, mc):
        ...
        self.needed_path_entries = [
            ...
            PathEntry('soundfont', _('SoundFont:'), _('Your preferred SoundFont (.sf2, .sf3)'), False, 'SoundFont (*.sf2;*.sf3)|*.sf2;*.sf3', self.soundfont_changed)
        ]
        ...
        for entry in self.needed_path_entries:
            ...
            if entry.add_default:
                path_choices = self.append_exe(self.get_default_path_for_executable(entry.name), path_choices)
...
#FAU 20240105: py2app sets also environment variable RESOURCEPATH to point to Resources folder
#FAU 20240105: Use of this information to either look for the libraries side to easy_abc or in the app folder
#FAU 2040107: redundant function with get_default_path_for_executables
#    def get_default_path(self, executable):
#        if wx.Platform == "__WXMSW__":
#            return os.path.join(cwd, 'bin', '%s.exe' % executable)
#        elif wx.Platform == "__WXMAC__":
#            if getattr(sys, "frozen", None) and os.environ.get('RESOURCEPATH'):
#                return os.path.join(os.path.dirname(os.environ["RESOURCEPATH"]),'Helpers', executable)
#            else:
...
class MainFrame(wx.Frame):
    def __init__(self, parent, ID, app_dir, settings, options):
        ...
        if platform.system() == 'Windows':
            ...
            if platform.system() == 'Darwin':
                default_soundfont_path = os.path.join(application_path, 'sound', 'FluidR3Mono_GM.sf3')
            else:
    ...
    def export_pdf_tunes(self, only_selected=False, single_file=False):
        ...
        if not gs_path and not gsapi_available:
            ...
            dlg.ShowModal()
            return
        if not os.path.exists(gs_path) and not gsapi_available:
    ...
    def restore_settings(self):
        ...
        if gs_path and (os.path.exists(gs_path) == False) and not gsapi_available:
